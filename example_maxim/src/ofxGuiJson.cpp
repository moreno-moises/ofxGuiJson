...
void ofxGuiJson::parseGuiElements() {
    ...
    try {
        ...
        for (const auto& el : config["gui_elements"]) {
            ...
            float posX = el["position"].value("x", 0);
            float posY = el["position"].value("y", 0);

            std::shared_ptr<ofxBaseGui> guiElement;

            if (type == "button") {
                auto button = std::make_shared<ofxButton>();
                button->setup(label, width, height);
            ...
            staticToggles.insert(id);  // Track all elements as static
...
std::shared_ptr<ofxBaseGui> ofxGuiJson::getGuiElement(const std::string& label) {
    auto it = guiElementsMap.find(label);
    if (it != guiElementsMap.end()) {
        return it->second;
    }
    return nullptr;
}
